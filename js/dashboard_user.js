...
async function loadArticles() {
  try {
    ...
    const journalsResponse = await fetch('/ksmaja/api/list_journals.php?limit=50&offset=0');
    const journalsData = await journalsResponse.json();
    
    // Fetch opinions from database (if you have opinions table)
    let opinionsData = { ok: false, results: [] };
    try {
      const opinionsResponse = await fetch('/ksmaja/api/list_opinion.php?limit=50&offset=0');
    ...
    if (journalsData.ok && journalsData.results) {
      journals = journalsData.results.map(j => {
        // Parse JSON fields
        const authors = j.authors ? (typeof j.authors === 'string' ? JSON.parse(j.authors) : j.authors) : [];
        const tags = j.tags ? (typeof j.tags === 'string' ? JSON.parse(j.tags) : j.tags) : [];
        
        return {
          id: j.id,
          title: j.title,
          judul: j.title, // Alias untuk backward compatibility
          abstract: j.abstract,
          abstrak: j.abstract, // Alias
          authors: authors,
          author: authors, // Alias
          penulis: authors.length > 0 ? authors[0] : 'Admin', // Alias
          tags: tags,
          date: j.created_at,
          uploadDate: j.created_at,
          fileData: j.file_url,
          coverImage: j.cover_url || 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=500&h=400&fit=crop',
          cover: j.cover_url,
          views: j.views || 0,
          type: 'jurnal'
        };
      });
      console.log(` Loaded ${journals.length} journals from database`);
    }
    ...
    if (opinionsData.ok && opinionsData.results) {
      ...
      console.log(` Loaded ${opinions.length} opinions from database`);
    }
      ...
      return `
        <div class="article-card" onclick="openArticleDetail('${article.id}', '${article.type}')" style="cursor: pointer;">
          <div class="article-image-container">
            <img src="${coverImage}" alt="${title}" class="article-image" onerror="this.src='https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=500&h=400&fit=crop'">
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  window.addEventListener('opinions:changed', async () => {
    console.log('📢 Opinions changed event received, reloading...');
    await renderArticles();
  });
});
